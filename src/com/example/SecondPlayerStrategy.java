...
public class SecondPlayerStrategy implements PlayerStrategy {
    public RunMeld runMeldCards;
    public SetMeld setMeldCards;
    ...
    public void receiveInitialHand(List<Card> hand) {
        for (int handIndex = 0; handIndex < hand.size(); handIndex++) {
            if (runMeldCards.canAppendCard(hand.get(handIndex))) {
                runMeldCards.appendCard(hand.get(handIndex));
            } else if (setMeldCards.canAppendCard(hand.get(handIndex))) {
                setMeldCards.appendCard(hand.get(handIndex));
            }
        }
        playersHand = hand;
    }
    ...
    public boolean willTakeTopDiscard(Card card) {
        ...
        if (runMeldCards.canAppendCard(card)) {
            takeCard = true;
        } else if (setMeldCards.canAppendCard(card)) {
            takeCard = true;
        }
        return takeCard;
    }
    ...
    public Card drawAndDiscard(Card drawnCard) {
        ...
        for (int handIndex = 0; handIndex < playersHand.size(); handIndex++) {
            if (runMeldCards.canRemoveCard(playersHand.get(handIndex))) {
